# app.py — 도로주행 자동 배정 (오전/오후 분리 · 1종수동 기본배정 · 혼합 최소화)
...
def parse_staff_text(src: str):
    ...
    sections = {
        ...
        "PERIOD": re.compile(r"^\s*[1-5]\s*교시\s*:", re.I),  # 교시 라인
    }
    out = {
        "classes": {"1M":[], "1A":[], "2A":[], "2M":[]},
        "course": set(),
...
def auto_assign(
    ...
    for p in range(1,6):
        ...
        # 3) 로컬 스왑: must_1m가 비-1M을 들고 있고, 다른 사람이 1M을 들고 있으면 교환 시도
        changed = True
        while changed:
            changed = False
