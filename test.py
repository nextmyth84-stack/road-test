# app.py — 도로주행(오전/오후 분리) 자동 배정 · Streamlit 단일 파일
...
def parse_staff_text(src: str):
    ...
    반환:
      {'classes': {'1M':[], '1A':[], '2A':[], '2M':[]}, 'course': set(), 'key': Optional[str]}
    """
    sections = {
        ...
        "PERIOD": re.compile(r"^\s*[1-5]\s*교시\s*:", re.I),  # 무시
    }
    out = {"classes": {"1M":[], "1A":[], "2A":[], "2M":[]}, "course": set(), "key": None}
    cur = None
...
# -----------------------
def auto_assign(
    staffs_by_period: Dict[int, List[Staff]],          # 교시별 근무자 리스트(오전/오후 분리 반영)
    ...
    # 코스점검 여부는 Staff.morning_course_check에 담겨 있음(오전/오후 텍스트 파서로 채움)
    course_names = set(n for lst in staffs_by_period.values() for n in [s.name for s in lst if s.morning_course_check])
    ...
    for p in range(1,6):
        ...
        for c in ("1M","2M"):
            ...
            _assign_units(
                ...
                u_s, options, unavoidable, header_cls_by_staff, mix_daily, mix_period
            )
            dem[c] -= need

        # 2) 잔여 수요 배정(1M→2M→1A→2A)
...
def _assign_units(
    ...
    for _ in range(units):
        # 1) 혼합 제약(헤더와 다르면 제한): 1) 교시 상한 0, 2) 일일 상한 1
        def can_mix(name: str) -> bool:
            return (mix_period[(p,name)] < options.mix_period_cap) and (mix_daily[name] < options.mix_daily_cap)

        cand_soft: List[Staff] = []
        cand_over: List[Staff] = []   # 소프트 상한(u_s) 초과 허용
        cand_relax_mix: List[Staff] = []  # 혼합 상한 완화가 필요한 후보(불가피용)
        ...
        for s in eligible:
            ...
            mix_ok = True
            if is_mix:
                mix_ok = can_mix(s.name)
            # 후보 분기
            if not mix_ok:
                # 혼합 제한 때문에 막힌 후보(불가피 시에만 사용)
                cand_relax_mix.append(s)
                continue
            # 혼합 통과
        ...
        def score(s: Staff):
            ...
            # 혼합이면 점수가 사실상 불리(상한도 막혀 있음). 점수에서는 추가 가산 없이 상한으로 제어.
            return (pseudo, cur_in_period, s.name)

        chosen_group = cand_soft or cand_over
        relaxed_mix = False
        if not chosen_group:
            # 혼합 상한 때문에 막힌 경우 → 불가피 완화: 교시 상한을 1까지, 일일 상한 1까지 열고 재시도
            # (이미 일일 상한=1) → 교시 상한만 1로 완화
            relaxed_mix = True
            cand_relaxed = []
            for s in cand_relax_mix:
                if cap_remain[s.name] <= 0:
                    continue
                # 교시 상한을 1까지 허용
                if mix_period[(p,s.name)] < max(1, options.mix_period_cap):
                    cur_in_period = sum(assignments[p][s.name].values())
                    if cur_in_period < u_s[s.name]:
                        cand_relaxed.append(s)
            if not cand_relaxed:
                # 그래도 없다면 초과까지 허용
                cand_relaxed = [s for s in cand_relax_mix if cap_remain[s.name] > 0]
...
st.title("도로주행 자동 배정 (오전/오후 분리 · 혼합 최소화)")
...
# 붙여넣기 파싱 → 오전/오후 근무자·코스점검자
am = parse_staff_text(am_text or "")
pm = parse_staff_text(pm_text or "")
am_staff_names = sorted(set(sum([am["classes"][c] for c in CLS_ORDER], [])))
pm_staff_names = sorted(set(sum([pm["classes"][c] for c in CLS_ORDER], [])))

# 수동 가능 명단(기본: 저장해둔 팀 + 텍스트의 1M/2M 등장 인원)
...
if run:
    try:
        assignments, loads, unavoidable = auto_assign(
            ...
            manual_only_counts=manual_only,
            options=Options(
